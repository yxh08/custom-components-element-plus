<template>
  <div style="height: 100%; display: flex; flex-direction: column">
    ...
    <div style="border: 1px solid var(--el-border-color)">
      ...
      <el-table v-bind="{ ...ElTableProps }" :data="tableData">
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const searchMethod_ = async (info = { page: 1, pagesize: 20 }) => {
      const data = {
        ...Object.fromEntries(
          formItems.value.map((x: any) => [x.prop, x.value]),
        ),
        ...info,
      }
      ...
      tableColumns.value = props.dict?.map((x: DictItem) => ({
        component: h(
          ElTableColumn,
          {
            ...
            ...(ColumnWidthMap[x.label]
              ? { width: ColumnWidthMap[x.label] }
              : {}),
          },
          {
            default: () =>
